...
async def create_incident(incident_data: CreateIncidentRequest) -> IncidentResponse:
...
async def update_incident(
    incident_id: UUID, incident_data: UpdateIncidentRequest
) -> IncidentResponse:
    return await incident_service.update_incident(incident_id, incident_data)


@router.delete("/{incident_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_incident(incident_id: UUID) -> None:
